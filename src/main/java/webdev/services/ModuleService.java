...
public class ModuleService {
	...
	public Module findModuleById(@PathVariable("moduleId") int moduleId) {
		...
		if (data.isPresent()) {
			return (Module) data.get();
		}
		return null;
	}

	// Finds module by course id.
	...
	public Module updateModule(@RequestBody Module module, @PathVariable("id") int moduleId) {
		Optional<Module> moduleData = moduleRepository.findById(moduleId);

		if (moduleData.isPresent()) {
			Module updateModule = moduleData.get();
